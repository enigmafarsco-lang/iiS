...
int remove_cb(const char *fpath, const struct stat *sb, int typeflag, FTW *ftwbuf)
{
    return remove(fpath);
}

int recursive_remove(char *dirpath)
{
...
void math_expression_objects_clean()
{
#ifdef linux
...
gboolean eval(const GMatchInfo *info, GString *res, gpointer data)
{
    gchar *match;
    gchar *replace;
    int index;
...
char *string_replace(const char *string, const char *pattern, const char *replacement, GRegexEvalCallback eval)
{
    GRegex *rex;
    gchar *result;
...
//    if (eval)
//        result =(gchar *) g_regex_replace_eval(rex, string, -1, 0, 0, eval, NULL, NULL);
//    else
...
char *c_file_create(const char *user_expression, GSList *basenames)
{
...
int shared_object_compile(char *base_filename)
{
...
math_expression_generator::math_expression_generator()
{
}
...
math_function math_expression_get_math_function(const char *expression_txt, void **lib_handler, GSList *basenames)
{
    ...
    *lib_handler = dlopen(dlopen_path, RTLD_LOCAL | RTLD_LAZY);
    ...
    math_fn = (math_function)dlsym(*lib_handler, MATH_FUNCTION_NAME);
    if (!math_fn) {
